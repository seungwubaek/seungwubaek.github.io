...
근데 우리가 블로그 내용을 수정하고 그것이 제대로 반영이 되었는지 확인해 보고 싶을 때마다 수정한 내용을 Repository에 업로드 하고 Github는 이를 다시 빌드하고 서비스하는 과정을 반복하도록 할것인가? No~~ 그러면 오래 걸릴 뿐더러 Github는 그렇게 쓰는 것이 아니고 너무나 비효율적이겠지. 그대신 우리는 블로그를 내 개인 컴퓨터에서 테스트로 띄워서 수정 내용을 빠르게 확인할 수 있도록 하겠다.
