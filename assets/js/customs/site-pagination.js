var currentSitePaginatorId = 'site-paginator-top';

const getCorrectPaginatorId = () => {
  if ($(window).outerWidth() >= siteVariables.screenLarge) {
    return 'site-paginator-top';
  } else {
    return 'site-paginator-bottom';
  }
};

const getPaginator = () => {
  currentSitePaginatorId = getCorrectPaginatorId();
  return $(`#${currentSitePaginatorId}`);
};

var $sitePaginator = getPaginator();
...
if ($sitePaginator.length > 0) {
  ...
  function sortPaginationData(sortKey) {
    let curSortKey = $('.recent-posts__opt.sort-dropdown').data(
      'current-sort-key'
    );
  ...
  function paginationTemplate(data, uiText) {
    ...
    for (post of data) {
      if (post.ad) {
        ...
        var dateLabelValISO,
          modifiedDateLabelValISO = '';
        if (post.date.length > 0) {
          dateLabelValISO = new Date(
            post.date.replace(/-/g, '/').replace('T', ' ')
          ).toISOString();
        }
        if (post.last_modified_at.length > 0) {
          modifiedDateLabelValISO = new Date(
            post.last_modified_at.replace(/-/g, '/').replace('T', ' ')
          ).toISOString();
        }
        ...
        for (categoryIdx of Array(post.categories.length).keys()) {
          ...
          categoryHtml.push(
            `<a href="${
              '/' + innerCategories.join('/') + '#wholetoc__title'
            }" class="page__header-taxonomy-item" rel="tag">${
              post.categories[categoryIdx]
            }</a>`
          );
        }
  ...
  function loadPaginator() {
    console.log($sitePaginator.attr('id'));
    ...
    if (!isEventAttached) {
      $('.paginationjs-go-button > input').click(function () {
        $sitePaginator.pagination(
          'go',
          $('.paginationjs-go-input > input').val()
        );
      });
      ...
      $('.paginationjs-go-input > input').focus(function () {
        $(this).select();
      });
  ...
  const switchPaginator = () => {
    // $sitePaginator가 이미 있는 상태에서 시작
    // const sitePaginatorIdToSwitch = getCorrectPaginatorId();
    const $sitePaginatorToSwitch = getPaginator();
    if ($sitePaginator.attr('id') !== $sitePaginatorToSwitch.attr('id')) {
      destroyPaginator();
      $sitePaginator = $sitePaginatorToSwitch;
      loadPaginator();
    }
  };
