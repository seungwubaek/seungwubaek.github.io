...
last_modified_at: "2023-12-16 02:06:00 +0900"
excerpt:
  "
  Django는 Python 기반의 풀스택 웹 프레임워크이다.
  Django만 있으면 Python 문법의 로직 구현을 통해 완전한 웹 어플리케이션을 개발할 수 있다.<br/>
  Java에게 Spring이 있다면 Python에게는 Django(장고)가 있다.
...
일단적으로 잘 알려진 소프트웨어 디자인 패턴 중 하나로 MVC(Model-View-Controller)가 있는데 MVT는 Django에서 만든 MVC의 변형이다.
...
보다 풍부한 스타일링을 위해 SASS 등의 CSS 전처리기도 도입할 수 있다.

(WSGI(Web Server Gateway Interface)는 Python 기반의 웹 어플리케이션이 공통적으로 사용하는 필수 레이어이므로 따로 다루지 않음)
...
이것은 위 [Python 기반의 유일무이한 풀스택 웹 프레임워크](#python-기반의-유일무이한-풀스택-웹-프레임워크)에서 설명한 처럼,
Django에 웹 개발에 필요한 다양한 기능들이 대부분 자체적으로 마련돼 있으므로 가능하다.
...
Python 기반의 웹 프레임워크에는 Django 외에 Flask, FastAPI 등이 있다.
이들도 유명하고 뛰어나지만 이들은 Micro Framework 이기 때문에 Django 만큼 웹 서비스를 위한 기능들을
...
예전에 나는 E-Commerce 웹 서비스 구축을 위해 Django와 FastAPI 중 하나를 백엔드 스택으로 선택해야 했었다.

Django에는 웹 서버의 다양한 기본 기능들이 마련되어 있지만 실사용을 위해서는 사용을 위한 설정 단계가 필요했다.<br/>
Django를 쓰지 않으면 굳이 필요하지 않은 설정 단계들을 생략하고 개발 시간을 단축할 수 있을 것이라 여겼다.

따라서 프로젝트가 아직 MVP 수준이었으므로 단순한 구조만 염두에 두고 가볍고 빠르다고 알려진 FastAPI를 선택했다.

그러나 결과적으로 FastAPI를 사용하는 것이 전체 개발 시간을 단축시켜주진 못했다.<br/>
FastAPI로 단순한 API를 구현하는 것은 쉬웠다. 하지만 그렇게 날것의 상태로 배포할 순 없었기 때문이다.

<div class="md-monologue">어쩌면 내가 바로 마감시간에 쫓기는 완벽주의자일지도 모른다.</div>

단순 코어 기능만 구현된 상태에서 Access Control 구현, 효과적인 유지보수를 위한 디자인 패턴 구현, 체계적인 DB 관리 등
조금만 더 살을 붙이려고 시도하니 해야할 일이 엄청나게 많아졌다.

FastAPI는 어떤 소프트웨어 디자인 패턴을 강제 하지 않기 때문에 스스로 결정해야 했다.<br/>
나는 MVC 디자인 패턴으로 레이어를 구성하기로 하고 각 단계에 맞는 모듈들을 직접 작성했다.

또한 Django가 제공하는 Django ORM이 존재하는 것과 달리 FastAPI ORM은 존재하지 않기 때문에
데이터 조작을 위해 SQLAlchemy를 연동시키고 DAO를 작성했다.

뒤이어 DTO, Service, Controller 등을 직접 구현해 나갔다.

이렇게 각 단계를 직접 설계하고 구현해나가다보니, 결국 Django를 이용하는 것 보다 더 많은 시간을 소비했다.<br/>
...
예를 들면, WebSocket 같이 자체 제공되지 않는 프로토콜을 사용하고 싶다던지,
